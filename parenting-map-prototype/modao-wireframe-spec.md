...
本版覆盖旧的“今日/百科/儿保/用品/学习”底部结构。新的产品方向是：

- `首页` 不再是今日任务流，而是孩子个人信息中枢。
- `用品` 和 `学习` 不再作为底部主栏目，统一收进 `百科` 的二级分类。
- 底部导航改为：`首页 / 百科 / 儿保 / 成长 / 我的`。
- 首页核心信息：出生日期、性别、城市、喂养方式、出生体重、出生体长、本次儿保体重、本次儿保身长。
- 首页核心输出：月龄、阶段、身高体重趋势提示、喂养方式对应建议、常用入口。
- 百科采用“电商/本地生活式分类入口”：先展示大类卡片，再进入小类标签和具体内容，避免一上来堆长文章。
...
- 首页：`GET /api/home`
- 百科分类：`GET /api/encyclopedia`
- 内容详情：`GET /api/articles/:id`
- 用品详情：`GET /api/products/:id`
- 儿保：`GET /api/care-schedule`
- 成长记录：`GET /api/growth-records`
- 搜索：`GET /api/search?q=`
